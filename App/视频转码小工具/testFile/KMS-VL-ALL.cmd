...
:: 设置 KMS 客户端参数
::===================
:: _KMSHost 参数可以是 (0-255.0-255.0-255.0-255)，但不是 127.x.x.x 或本地主机 IP [离线模式]；KMS 服务器名称/IP [在线模式]
...
:: 获取基于活动 CBS 包的版本，或者退回到 Dism CurrentEdition
...
if %_WinBuild% GEQ 9600 (
  REM 在 echo 消息中没有附带内容或括号
    echo.&echo 当前操作系统：Windows 10
  goto :Win8.1AndLater
) else if %_WinBuild% GEQ 2600 (
    echo.&echo 当前操作系统：Windows XP/7/8
  goto :Win8AndEarlier
) else (
...
if %_OfflineMode% EQU 1 (
  ...
  REM Windows Vista 不支持 SetKeyManagementServicePort，因此还原为默认的 KMS 端口
...
REM 由于我们使用外部 KMS 服务器，因此将 KMS_Emulation 设置为 0；1 用于注入文件中的内部 KMS 服务器
...
wmic path %_MicrosoftProduct% where (Description like '%%KMSCLIENT%%') get Name /value %_Nul_2% | findstr /i "Office" %_Nul_1% && (
  ...
  if %_WinBuild% GEQ 9200 (
    echo.
    echo 未检测到 Office 2013 及以上版本的 VL 产品；零售版本需要先转换为 VL 版本。
  )
)
::===================
:: 检查已安装的 Windows 是否支持 KMS 激活；如果没有 VolumeLicensed 的 Windows 或 Office 则退出
...
::=============================================================
:CheckProduct
:: 如果已检测到 KMS 客户端早期已经激活或者已有 GVLK，调用激活功能
...
:: 对于五合一的 Windows 10 安装程序，程序将不再执行
...
echo.
echo 名称为 %_Name% 的 GVLK
echo 未找到 SKU-ID 为 %1 
echo.
echo 如果本次激活失败了，请手动为该产品输入对应的 GVLK 并重新运行 KMS_VL_ALL
call :Activate %1
exit /b
::=============================================================
:InstallKey
